...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span className={cn(
      "px-2.5 py-1 rounded-md text-[10px] font-medium tracking-tight border shadow-sm cursor-default",
      currentStyle
    )}>
      {displayStatus}
    </span>
...
export default function ApplicantsClient({ initialData }: ApplicantsClientProps) {
  ...
  const rejectApplication = async (appId: number, reason?: string) => {
    setLoading(appId);
    try {
      const res = await dashboardServerFetch(`employer/reject/${appId}`, {
        method: "PATCH",
        // Pass the reason to the API, though user says they will update the backend later
  ...
  return (
    <div className="max-w-6xl mx-auto px-2 py-2 space-y-2 overflow-x-hidden">
      {!selectedApplicant ? (
        <>
          {/* Compact Header */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-2 border-gray-200">
            <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
            {[
              ...
              <div key={i} className={cn(
                "bg-white p-2 md:p-2.5 rounded-lg border border-slate-300 shadow-sm flex items-center gap-2 hover:shadow-md transition-shadow",
          ...
          <div className="grid grid-cols-1 gap-1.5">
            {filteredApps.length > 0 ? (
              filteredApps.map((app) => (
                <div
                  key={app.id}
                  className="bg-white rounded-lg border border-slate-300 shadow-sm hover:shadow-md transition-all p-2 md:p-2.5 flex flex-col md:flex-row gap-2 relative group overflow-hidden"
                >
                  <div className="flex gap-4 items-start flex-1 min-w-0">
                    <div className="w-8 h-8 md:w-9 md:h-9 rounded-lg bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-600 font-normal text-xs shrink-0 overflow-hidden shadow-inner group-hover:scale-105 transition-transform">
                  ...
                  <div className="flex items-center gap-2 pt-2 md:pt-0 border-t md:border-t-0 md:border-l border-slate-300 md:pl-2">
                    <Button
                      variant="outline"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
            <div className="flex items-center gap-4">
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-bold text-slate-600 hover:text-slate-900 transition-colors bg-white hover:bg-slate-50 px-3.5 py-2 rounded-lg border border-slate-200 shadow-sm cursor-pointer shrink-0"
              >
                <ChevronLeft className="w-4 h-4" /> Back
              </button>
              {/* <div className="flex flex-col border-l border-slate-300 pl-4">
                <h2 className="text-lg font-bold text-slate-900 leading-none mb-1">{getCandidateName(selectedApplicant)}</h2>
                <p className="text-xs font-medium text-slate-500">Candidate Profile</p>
              </div> */}
            </div>
            ...
            <div className="flex items-center gap-2">
              <select
                className={cn(
                  "h-9 px-3 w-32 rounded-lg bg-white border border-slate-300 text-xs font-medium text-slate-700 focus:ring-1 focus:ring-primary outline-none cursor-pointer hover:bg-slate-50 transition-colors shadow-sm",
              ...
              <Button
                className="h-9 px-4 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100 hover:text-rose-700 font-medium text-xs transition-all flex items-center justify-center gap-1.5 shadow-sm border border-rose-200 disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              {selectedApplicant.status?.toLowerCase() !== 'rejected' && (
                <Button
                  className="h-9 px-5 rounded-lg bg-emerald-600 text-white font-medium text-xs shadow-md shadow-emerald-200/50 hover:bg-emerald-700 transition-all flex items-center justify-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-emerald-700/50"
                ...
                {selectedApplicantFullData.job_seeker?.experiences?.length > 0 && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-base font-bold text-slate-900 tracking-tight">Experience</h3>
                    </div>
                      ...
                      {selectedApplicantFullData.job_seeker.experiences.map((exp: any) => (
                        <div key={exp.id} className="p-4 rounded-xl border border-slate-300 bg-white shadow-sm hover:border-slate-400 transition-colors">
                          <div className="flex justify-between items-start mb-1">
                            ...
                            <span className="text-xs font-medium text-slate-900 bg-slate-50 px-3 py-1.5 rounded-md border border-slate-300 tracking-tight shrink-0 ml-2">
                ...
                {selectedApplicantFullData.job_seeker?.educations?.length > 0 && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-base font-bold text-slate-900 tracking-tight">Education</h3>
                    </div>
                    <div className="space-y-2.5">
                ...
                {selectedApplicantFullData.job_seeker?.skills?.length > 0 && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-base font-bold text-slate-900 tracking-tight">Skills</h3>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {selectedApplicantFullData.job_seeker.skills.map((skill: any, idx: number) => (
                        <span key={idx} className="px-3.5 py-1.5 text-xs font-medium tracking-tight bg-slate-50 border border-slate-300 rounded-lg text-slate-900">
            ...
            {selectedApplicantFullData?.answers?.length > 0 && (
              <div className="space-y-3 pt-2">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-base font-bold text-slate-900 tracking-tight">Questionnaire</h3>
                </div>
                <div className="space-y-2.5">
            ...
            <div className="space-y-4 pt-1">
              <div className="flex items-center justify-start gap-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-base font-bold text-slate-900 tracking-tight">Resume</h3>
                </div>
                {(selectedApplicantFullData?.resume?.file_url || selectedApplicant.resume?.file_url) && (
                  <Button
                    ...
                    className="h-8.5 px-3 rounded-lg text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white flex items-center gap-1.5 shadow-md hover:shadow-lg transition-all border-none"
                  >
                    <Download className="w-4 h-4 text-white" /> Download Resume
                  </Button>
                )}
              </div>
              ...
              <div className="space-y-3">
                {(selectedApplicantFullData?.resume?.file_url || selectedApplicant.resume?.file_url) ? (
                  <div className="max-w-sm w-full space-y-3">
                    <div className="rounded-2xl border border-slate-300 bg-white p-1 overflow-hidden aspect-[1/1.414] group relative shadow-inner">
                      <iframe
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl border border-slate-200 overflow-hidden">
            ...
            <div className="p-5 space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-tight">Rejection Reason</label>
                <textarea
                  value={rejectReason}
                  onChange={(e) => setRejectReason(e.target.value)}
                  placeholder="Enter reason for rejection (optional)"
                  className="w-full min-h-[100px] p-3 text-sm rounded-xl border border-slate-200 focus:border-rose-300 focus:ring-4 focus:ring-rose-50 transition-all outline-none resize-none"
                />
              </div>
            </div>
            <div className="p-5 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3">
              ...
              <Button
                className="h-9 px-5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-semibold shadow-md shadow-rose-200"
                onClick={() => rejectApplication(selectedApplicant.id, rejectReason)}
                disabled={loading === selectedApplicant.id}
              >
                {loading === selectedApplicant.id ? (
