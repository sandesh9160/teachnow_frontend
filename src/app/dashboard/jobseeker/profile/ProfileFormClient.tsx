...
function mapServerProfile(initial: Record<string, unknown>) {
  return {
    name: String(initial.name ?? initial.full_name ?? ""),
...
export default function ProfileFormClient({
  ...
  const [profileData, setProfileData] = useState(() => mapServerProfile(initialProfile));
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      try {
        ...
        if (cancelled) return;
        setProfileData({
          name: String(profile.name ?? profile.full_name ?? ""),
        ...
        /* keep server-provided initialProfile */
      } finally {
