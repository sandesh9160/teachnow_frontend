...
export default function ProfilePage() {
  ...
  const { createEducation, updateEducation, deleteEducation } = useEducation();
  ...
  const handleEduSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      if (editingEduId) {
        await updateEducation(editingEduId, eduFormData);
        toast.success("Education updated!");
      } else {
        await createEducation(eduFormData);
        toast.success("Education added!");
      }
      setEduFormData({ institution: "", degree: "", field_of_study: "", start_date: "", end_date: "", description: "" });
      setShowEduForm(false);
      setEditingEduId(null);
      // Refresh profile to get updated education list
      const data = await getProfile();
      setEducation(data?.data?.education || data?.education || []);
    } catch (err) {
            ...
            education.map((edu) => (
              <div key={edu.id} className="p-6 flex justify-between items-start group hover:bg-gray-50/50 transition">
                ...
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => handleEduEdit(edu)} className="p-2 text-gray-400 hover:text-primary rounded-lg transition"><Edit2 className="w-4 h-4" /></button>
                  <button onClick={() => handleEduDelete(edu.id)} className="p-2 text-gray-400 hover:text-red-500 rounded-lg transition"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            ))
          ) : (
